...
// const selectedDate = computed(() => convertToMMDDYYYY(props.modelValue)) 
// const selectedDate = computed(() => (props.modelValue ? convertToMMDDYYYY(props.modelValue) : ''))  
...
watch(
  () => props.modelValue,
  (newVal) => {
    selectedDate.value = newVal ? convertToMMDDYYYY(newVal) : ''
  }
)
...
<template>
  ...
  <div class="relative">
    <input
      class="datepicker w-full h-[50px] rounded py-3 px-5 font-normal outline-none transition border-0 shadow-1 focus:ring-primary"
